...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'No Video':
						ClientPrefs.noVideo = !ClientPrefs.noVideo;

					case 'Virgin-Mode':
						ClientPrefs.virginMode = !ClientPrefs.virginMode;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'No Video':
				daText = "If checked, the video will get removed from your screen.";
			case 'Virgin-Mode':
				daText = "If checked, the cam hud will be hidden\nand the botplay will be turned on.";
		}
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'No Video':
						daValue = ClientPrefs.noVideo;
					case 'Virgin-Mode':
						daValue = ClientPrefs.virginMode;
				}
